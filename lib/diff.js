...
function buildReviewModel(repoRoot, reviewMode) {
  ...
  // Accumulate shows every uncommitted change in the working tree vs the
  // last git commit (HEAD), regardless of which session or prompt created
  // it — the "all the Claude work I haven't committed yet" view.
  // Per-request stays scoped to the session/prompt baseline and to files
  // the transcript says Claude actually touched.
