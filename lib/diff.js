...
function buildReviewModel(repoRoot, reviewMode) {
  ...
  // Accumulate mirrors `git status`: every uncommitted change against
  // HEAD, tracked or untracked, regardless of who made it or when — not
  // scoped to Claude's edits or any particular prompt. Per-request stays
  // scoped to the session/prompt baseline and to files the transcript
  // says Claude actually touched.
