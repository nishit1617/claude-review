...
async function openReview(repoRoot, { manual, hookInput } = {}) {
  ...
  const model = buildReviewModel(repoRoot, currentMode(repoRoot));
...
function serve(repoRoot) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && req.url === "/api/sig") {
      ...
      res.end(JSON.stringify({ sig: reviewSig(repoRoot), version: VERSION, mode: currentMode(repoRoot) }));
      return;
    }
    ...
    if (req.method === "GET" && req.url === "/api/mode") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ mode: currentMode(repoRoot) }));
      return;
    }
    if (req.method === "POST" && req.url === "/api/mode") {
      ...
      req.on("end", () => {
        try {
          ...
          if (mode !== "per-request" && mode !== "accumulate") {
          ...
          log(repoRoot, `reviewMode set to ${mode}`);
          res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (req.method === "POST" && req.url === "/api/submit") {
      ...
      req.on("end", () => {
        try {
          ...
          // Recomputed fresh so hunk ids/line indices match exactly
          // what /api/review just showed — anything else risks
          // reverting the wrong lines.
          const model = buildReviewModel(repoRoot, currentMode(repoRoot));
