...
/*
 * Content-based selective revert. Instead of `git apply -R` (whole
 * hunks only), we rebuild the affected region of each file directly,
 * which allows reverting individual lines inside a hunk.
 *
 * selections: { [hunkId]: { lines: [indices into hunk.lines] } }
 *   An index present = that +/- line is REJECTED (reverted).
 *
 * Walk a hunk's lines against the current file region:
 *   ' ' context      -> keep the file's line
 *   '+' addition     -> rejected? drop it : keep the file's line
 *   '-' deletion     -> rejected? re-insert the old line : leave deleted
 * Hunks are processed bottom-up per file so line numbers stay valid.
 */
function revertSelection(repoRoot, model, selections, reasons) {
  ...
  for (const file of model.files) {
    ...
    for (const h of ordered) {
      const rejected = new Set(selections[h.id].lines);
      ...
      for (let i = 0; i < h.lines.length; i++) {
        ...
        if (p === " ") {
          ...
          if (!rejected.has(i)) rebuilt.push(lines[ptr]);
          ptr++;
        } else if (p === "-") {
          if (rejected.has(i)) rebuilt.push(l.slice(1).replace(/\r$/, ""));
        }
      }
      ...
      log(repoRoot, `revert ok ${file.filePath} ${h.id} (${rejected.size} lines)`);
    }

    if (lines.length === 0 && file.created) {
      // Fully rejected new file: remove it.
      try { fs.unlinkSync(abs); } catch (_) {}
      log(repoRoot, `deleted fully-rejected new file ${file.filePath}`);
    } else {
