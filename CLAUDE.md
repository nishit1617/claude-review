...
bin/claude-review.js                CLI entry point (snapshot / prompt / open / review / serve)

.claude-plugin/plugin.json, marketplace.json   plugin manifests
package.json                         version, currently 1.0.3

Runtime state lives in .claude-review/ inside whichever project the plugin is reviewing (not in the plugin's own folder), and is git-excluded automatically:
...
server.pid              current server's process id; also used to detect whether a prior
                         server recently ran here, for the auto-open timing decision
